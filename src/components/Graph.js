...
export default function Graph(props) {
	const { data, chise, current, setInput, debounceSetSearchTerm, dimensions, normalView, mobile } = props;
	...
	const handleHover = (node, prevNode) => {
		// node ? setNodeHovered(true) : setNodeHovered(false);
	...
	return (
		<GraphWrapper dimensions={dimensions} onClick={normalView} $nodeHovered={nodeHovered}>
			<ForceGraph3D
...
const GraphWrapper = styled.div`
	...
	cursor: ${(props) => (props.$nodeHovered ? 'pointer' : 'auto')};
