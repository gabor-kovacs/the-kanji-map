...
export const App: React.FC<Props> = ({ chise, data }) => {
  ...
  const [current, setCurrent] = useState<ChiseEntry>({
    kanji: "",
    structure: [],
    composition: [],
  });

  const [kanjiHistory, setKanjiHistory] = useState<(string | null)[]>(
    Array(4).fill(null)
  );
  ...
  useEffect(() => {
    ...
    if (
      current?.kanji !== "" &&
      current?.kanji &&
      !kanjiHistory.includes(current?.kanji)
    ) {
  ...
  //////////////////////////////////////////////////////////////////
  const [drawInputOpen, setDrawInputOpen] = useState<boolean>(false);
  ...
  const [layoutView, setLayoutView] = useState<Layout>({
    kanjiFocused: false,
  ...
  const normalView = () => {
    if (mobile) {
      ...
      if (
        layoutView.kanjiFocused ||
        layoutView.examplesFocused ||
        layoutView.radicalFocused
      ) {
  ...
  const mobile: boolean = useMediaQuery("(max-width:999px)");
  const smallmobile: boolean = useMediaQuery("(max-width:360px)");
  const desktop: boolean = useMediaQuery("(min-width:1000px)");
...
const MobileLayout = styled(animated.div)`
  ...
  background-size: cover, 100px 50px, 100px 50px, 100px 50px, 100px 50px,
    100px 50px;
...
const DesktopLayout = styled.div`
  ...
  background-size: cover, 150px 75px, 150px 75px, 150px 75px, 150px 75px,
    150px 75px;
